...
class User_model extends CI_Model
{
	...
	public function save_user($auth=false,$more=false)
	{
		# code...
		if($auth){
				if (is_array($auth)) {
					# code...
					$auth = $this->esc_array($auth);
					$email = $auth['email'];
					...
					if($more){
						...
						$other = $this->other_user_info($info);
	...
	function other_user_info($data=false)
	{
		//var_dump($data);exit();
		...
		if($data){
			if (is_array($data)) {
				foreach ($data as $key) {
					# code...
				//	print_r($key);
				$this->db->insert('more_user_info',$key);
				}
